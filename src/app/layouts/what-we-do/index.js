const WhatWeDo = () => {
    return (
        <div className='w-full bg-what-we-do xl:py-[18.5vh] py-[32px] flex flex-col justify-center items-center overflow-hidden sm:bg-cover xl:bg-cover bg-cover bg-right-bottom'>
            <p className='futura-book text-white uppercase tracking-widest xl:text-[4.45vw] xl:leading-[4.45vw] text-[24px] leading-[24px]'>
                What we do
            </p>
            <div className='text-white futura-book text-center xl:mt-[4vh] mt-[16px] xl:text-[1.8vw] text-[16px]'>
                <p>At Metaoptics Technologies, we are breaking through</p>
                <p>current lens limitations in consumer and IoT</p>
                <p>applications. Our high-throughput meta lens</p>
                <p>production ensures efficiency and cost-effectiveness.</p>
                <p>As AR/VR and HUD technologies rapidly expands, our</p>
                <p>innovative meta lens are essential to provide</p>
                <p>miniaturization in supporting this market growth.</p>
            </div>
        </div>
    );
};

export default WhatWeDo;
